...
export const create = async (user, args) => {
  ...
  if (!isAdmin && (args.user.role === 'admin' || args.user.role === 'editor')) {
    return Promise.reject(new PrivilegeError('Need admin privilege to create that user type'));
  }
 ...
 */
export const findOne = (user, args) => User.findOne({ _id: args.id });
...
export const find = async (user, args) => {
  if (user.role !== 'admin') return Promise.reject(new PrivilegeError('Need admin privilege to list users'));

  const totalUsers = await User.count();
  const users = await User.find().skip(args.per_page * args.page).limit(args.per_page);
  return Promise.resolve({ totalUsers, users });
};
